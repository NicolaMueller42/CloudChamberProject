...
# This script trains the neural network with transfer learning.
...
# Afterwards the architecture of the neural network is defined and it loads weights of the layers
#  of the pretrained NN and freezes the layers of the encoder part. Then the NN is trained.
...
# pairs = make_pairs("C:/Users/Nicola/Desktop/Uni/MLproject")
batch_size = 1
...
# this locks the weights of the encoder part of the neural network
# the pretrained NN already learned to extract important information from the images
# so we want our NN to focus on decoding the compressed information into masks
# ACTUALLY NOT FREEZING THESE LAYERS IS BETTER
for i in range(38):
...
unet.compile(optimizer=optimizer, loss=pce_dice_loss, metrics=[dice_coef])
...
metrics_logger = tf.keras.callbacks.TensorBoard(log_dir="./transfer_learning_logs", update_freq='epoch', write_images=True)

# trains it
unet_history = unet.fit(x=train_generator, epochs=40, steps_per_epoch=steps_per_epoch, callbacks=[metrics_logger])
...
unet.save("transfer_learning.h5")
